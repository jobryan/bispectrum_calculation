...
@summary:   Calculate Kl^(2,2) based off of Eqn. 75 given in 
...
@outputs:   Analytical kurtosis estimators,
            ...
            output/kl_22_ana_[i_num_r_trunc]_rsteps_[i_num_L]_Lsteps_[i_lmax]
                _lmax.dat

@command:   Compile with:

            /home/jobryan/bin/mpic++ kl21_ana.cpp -o kl21_ana -std=c++11
            ...
            /home/jobryan/bin/mpirun -np 12 ./kl21_ana 10 10 50

            If running from an incomplete run, (or if you want to start at an 
            intermediate point), in the location s_fn_cur_job 
            (../output/vi_cur_job_kl22.dat), there should be a text file with 5 lines
            ...
            If that file exists, kl21_ana will use its values as its starting
            location.
...
int main(int i_argc, char* s_argv[]){
    ...
    string s_fn_cur_job = "../output/vi_cur_job_kl22.dat";
    ...
    */
    ...
    // Filenames for kl22 and ell outputs
    ...
    o_strs3 << "../output/ell_out_" << i_num_r_trunc << "_rsteps_" 
           << i_num_ell_trunc << "_Lsteps_" << i_lmax << "_lmax_"
           << (int)d_gnl << "_gnl_" << (int)d_tnl << "_tnl_kl22.dat";
...
/*    int i_all_counter = 0;
    ...
    int ai_dims[] = {i_num_L, i_lmax, i_lmax, i_lmax, i_lmax};
    ...
    if (i_rank == 0) {
        ...
        double d_l1, d_l2, d_l3, d_l4, d_L, d_r_sum_22;
        vector<double> vd_result(11,0); // il1, il2, il3, il4, iL, dl1, dl2, dl3, dl4, dL, d_r_sum_22
        ...
        for (int i_rank_out = 1; i_rank_out < i_size; i_rank_out++) {
            ...
            if (i_continuing_job > 0){
                ...
                int ai_job[] = {
                    i_L_start + ((i_L_start + i_rank_out-1) / i_num_l / i_num_l / i_num_l / i_num_l) % i_num_L,
                    ...
                    (i_l4_start + i_rank_out-1) % i_num_l};
        ...
        if (i_continuing_job > 0){
            ...
            int ai_next_job[] = {
                    i_L_start + ((i_L_start + i_size-1) / i_num_l / i_num_l / i_num_l / i_num_l) % i_num_L,
...
/*                                    i_all_counter++;
                                    ...
                                    /* Check timer: if time > 15.5 hrs, write kl22 to file;
                                    ...
                                    if ((i_early_write == 0) && (d_time_elapsed > d_time_stop)) {
                                        ...
                                        cout << "Saving kurtosis power spectrum (2,2) to " 
                                             << s_fn_kl22 << endl;
...
/*            cout << "job (int): " << i_l1ind << "," << i_l2ind << "," 
            ...
            if ((fabs(d_l1 - d_l2) <= d_L) && (fabs(d_l1 + d_l2) >= d_L)){
                ...
                if ((fabs(d_l3 - d_l4) <= d_L) && (fabs(d_l3 + d_l4) >= d_L)){
                    ...
                    d_r_sum_22 *= sqrt((2*d_l1+1)*(2*d_l2+1)*(2*d_L+1) 
                        / (4 * M_PI))
                    ...
                    * w3j((int) d_l3, (int) d_l4, (int) d_L, vd_lnfa);
                    ...
                    d_r_sum_22 = pow(d_r_sum_22,2) / vd_cltt[i_l1ind] 
                        / vd_cltt[i_l2ind] / vd_cltt[i_l3ind] / vd_cltt[i_l4ind];
...
/*                    if (d_r_sum_tmp > 1.){
                    ...
                    d_r_sum_22 = d_r_sum_22 / (2*d_L + 1) / (2*d_L + 1);
            ...
            double ad_data[] = {(double)i_l1ind, (double)i_l2ind, 
                        ...
                        d_l1, d_l2, d_l3, d_l4, d_L, d_r_sum_22};
